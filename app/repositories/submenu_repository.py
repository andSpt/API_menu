...
class SubmenuRepository:
    ...
    async def _check_exists_submenu_by_attr(
        self, submenu_create: SubmenuCreate, menu_id: UUID, attr: str
    ) -> None:
        """Checking the menu object with the title attribute in the DB."""
        stmt = select(Submenu).filter(
    ...
    async def get_all_submenus(self, menu_id: UUID) -> list[SubmenuResponse]:
        stmt = self._get_basic_query_submenus().filter(Submenu.menu_id == menu_id)
        result: Result = await self.session.execute(stmt)
        list_submenus = [
            SubmenuResponse.model_validate(row, from_attributes=True) for row in result
        ]
        return list_submenus

    async def get_submenu(self, menu_id: UUID, submenu_id: UUID) -> SubmenuResponse:
        stmt = self._get_basic_query_submenus().filter(
    ...
    async def create_submenu(
        ...
        await self._check_exists_submenu_by_attr(
            menu_id=menu_id, submenu_create=submenu_create, attr="title"
        )
    ...
    async def update_submenu(
        ...
        submenu: SubmenuResponse = await self.get_submenu(
